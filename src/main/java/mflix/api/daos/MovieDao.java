...
public class MovieDao extends AbstractMFlixDao {
    ...
    private Bson buildLookupStage() {
        ...
        Bson expression = Document.parse("{'$expr': {'$eq': ['$movie_id', '$$id']}}");
        Bson lookup = Aggregates.match(expression);

        List<Bson> pipeline = new ArrayList<>();
        Bson sort = Aggregates.sort(Sorts.descending("date"));
        pipeline.add(lookup);
        pipeline.add(sort);
        return Aggregates.lookup("comments", variables, pipeline, "comments");
    ...
    public Document getMovie(String movieId) {
        ...
        return moviesCollection.aggregate(pipeline).batchSize(1).iterator().tryNext();
    }
    ...
    public List<Document> getMoviesByCountry(String... country) {
        Bson queryFilter = all("countries", Arrays.asList(country));
    ...
    public List<Document> getMoviesByText(int limit, int skip, String keywords) {
        ...
        List<Document> movies = new ArrayList<>();
        moviesCollection.find(textFilter)
                .projection(projection)
                .sort(sort)
                .skip(skip)
                .limit(limit)
